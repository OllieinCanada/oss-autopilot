...
export function isEntryPoint(entryPath: string | undefined): boolean {
  const normalizedPath = entryPath?.replaceAll('\\', '/');
  return Boolean(normalizedPath && ENTRY_SUFFIXES.some((suffix) => normalizedPath.endsWith(suffix)));
}
